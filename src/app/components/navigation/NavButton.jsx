...
const getIcon = (icon) => {
  switch (icon) {
    ...
    case "resume":
    return <NotepadText className='w-full h-auto' strokeWidth={1.5} />
    default:
    return <Home className='w-full h-auto' strokeWidth={1.5} />
  }
}

const NavButton = ({x, y, label, link, icon, newTab, labelDiretion="right"}) => {
return (
    <ResponsiveComponent> 
    ...
    {({size}) => {
      ...
      <span className= "peer bg-transparent absolute top-0 left-0 w-full h-full"/>
      <span className={clsx("absolute hidden peer-hover:block px-2 py-1 left-full mx-2 top-1/2 -translate-y-1/2 bg-background text-foreground text-sm rounded-md shadow-lg whitespace-nowrap", labelDiretion === "left" ? "right-full left-auto" : "")}> {label} </span>
